...
\section{Adding Hard Drive Space}

Expanding the virtual disk for a Windows VM in Proxmox involves both adjusting settings in Proxmox and then making changes within the Windows operating system. Follow the steps below to achieve this:

\begin{enumerate}[label=\arabic*.]
    \item \textit{Expand the Virtual Disk in Proxmox}:
    \begin{itemize}
        \item Navigate to Proxmox Web UI.
        \item Select the target VM.
        \item Navigate to \textit{Hardware} $\to$ \textit{Hard Disk} $\to$ \textit{Resize}.
    \end{itemize}
    
    \item \textit{Expand the Disk Inside Windows}:
    \begin{itemize}
        \item Press `Windows + R', type \texttt{diskmgmt.msc}, and press Enter to open \textit{Disk Management}.
        \item Locate your main partition (e.g., C:) and the segment of unallocated space in the graphical representation at the bottom.
        \item Right-click on the main partition and select \textit{Extend Volume}.
        \item Follow the \textit{Extend Volume Wizard}, usually accepting the default values.
        \item Upon completion, the main partition should utilize the newly added space.
    \end{itemize}
\end{enumerate}
